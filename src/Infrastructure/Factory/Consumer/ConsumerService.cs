...
{
    public async Task<Domain.Consumer.Consumer> GetByComplaintIdAsync(string complaintId)
    {
        ...
        SqlMapper.GridReader result = await unitOfWork.Connection.QueryMultipleAsync(ConsumerStoreProcedureNames.GET_CUSTOMER_FULL_DETAIL_BY_COMPLAINTID, param, transaction: unitOfWork.Transaction, commandType: CommandType.StoredProcedure);
